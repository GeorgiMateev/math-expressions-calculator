    ...
    public class CalculateExpressions
    {
        ...
        public static Queue<object> ConvertExpressionToRPN(string s)
        {
            ...
            try
            {
                while (index < str.Length)
                {
                    ...
                    if (isOperator)
                    {
                        if (symbol == '-')
                        {
                            bool isNumber2 = (int)str[index - 1] >= 48 &&
                                (int)str[index - 1] <= 57;
            ...
            catch (IndexOutOfRangeException e)
            {
                throw new FormatException
                    ("The input expression is not in a correct format!", e);
            }
        ...
        private static void ProceedWithRightParenthesis(Stack<string> stack, Queue<object> queue)
        {
            while (stack.Peek() != '('.ToString())
            {
                ...
                {
                    throw new FormatException
                        ("There are mismatched parentheses in the expression!");
                }
            }
            stack.Pop();
            if (stack.Count > 0)
            {
                bool isFunction =
                    ((int)stack.Peek()[0] >= 97 && (int)stack.Peek()[0] <= 122) ||
                    ((int)stack.Peek()[0] >= 65 && (int)stack.Peek()[0] <= 90);
                if (isFunction)
                {
                    string item = stack.Pop();
                    queue.Enqueue(item);
        ...
        private static void ProceedWithOperator
            (Stack<string> stack, Queue<object> queue, char symbol)
        {
            if (stack.Count > 0)
            {
        ...
        private static void ProceedWithComma(Stack<string> stack, Queue<object> queue)
        {
            ...
            {
                throw new FormatException
                    ("There is misplaced function separator or mismatched parentheses in the expression!");
            }
        }
        ...
        {
            double result = Math.Pow((double)list[index - 2],
                (double)list[index - 1]);
            list[index] = result;
